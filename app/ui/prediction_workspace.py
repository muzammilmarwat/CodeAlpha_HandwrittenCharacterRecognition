...
from app.services.saliency_service import overlay_saliency_on_processed_image
...
def render_preprocessing_workspace(result: PredictionResult) -> None:
    ...
    col_original, col_processed, col_saliency, col_overlay = st.columns(4)
    with col_original:
    ...
    with col_overlay:
        st.markdown('<div class="info-card">', unsafe_allow_html=True)
        st.caption("Saliency Overlay")
        if result.saliency_map is not None:
            st.image(
                overlay_saliency_on_processed_image(result.processed_image, result.saliency_map),
                clamp=True,
                use_container_width=True,
            )
        else:
            st.warning("Overlay unavailable.")
        st.markdown("</div>", unsafe_allow_html=True)
